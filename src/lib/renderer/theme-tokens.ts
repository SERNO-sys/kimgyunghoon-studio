/**
 * AWIE V2 - Theme token resolution (server-safe).
 *
 * This module contains the pure, server-safe token-resolution logic that maps
 * the raw ThemeConfig.resources.settings into structured ThemeTokens (colors,
 * spacing, typography, radius, shadows).
 *
 * SERVER-SAFE: This module MUST NOT contain 'use client', React hooks, browser
 * APIs, or any client-only code. It is imported directly by Server Component
 * pages (src/app/sites/[siteId]/page.tsx and [...slug]/page.tsx) so the pure
 * resolver can be invoked on the server.
 *
 * IMMUTABILITY: The resolver MUST NEVER mutate the ThemeConfig. It reads the
 * config and produces a derived, read-only ThemeTokens object.
 *
