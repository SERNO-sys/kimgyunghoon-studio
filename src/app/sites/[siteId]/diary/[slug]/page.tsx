...
export async function generateMetadata({
  ...
  const db = getDb();
  const post = data ? await getPostBySlug(db, data.site.id, slug) : null;

  if (!post) {
    return { title: `Not Found | ${data?.site?.name ?? 'Site'}` };
  }
...
export default async function DiaryDetailPage({
  ...
  const db = getDb();
  const post = await getPostBySlug(db, data.site.id, slug);
  if (!post) {
    notFound();
  }
